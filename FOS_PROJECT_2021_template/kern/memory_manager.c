...
void remove_from_LRU(struct Env* e, uint32 virtual_address, uint32 size,bool active) {
	...
	LIST_FOREACH(ele,&(find))
	{
		uint32*pagetable = NULL;
		if (ele->virtual_address >= virtual_address && ele->virtual_address < (virtual_address + size)) {
			LIST_REMOVE(&(e->ActiveList), ele);
			ele->empty = 1;
			LIST_INSERT_HEAD(&(e->PageWorkingSetList), ele);
			unmap_frame(e->env_page_directory, (void*) ele->virtual_address);
			if (LIST_SIZE(&(e->SecondList)) != 0&&active==1) {
				struct WorkingSetElement*l_in_sec = LIST_LAST(&(e->SecondList));
				LIST_REMOVE(&(e->SecondList), l_in_sec);
				LIST_INSERT_HEAD(&(e->ActiveList), l_in_sec);
				pt_set_page_permissions(e, l_in_sec->virtual_address,
				PERM_PRESENT | PERM_WRITEABLE, 0);
			}
			pt_clear_page_table_entry(e, ele->virtual_address);
			 get_frame_info(e->env_page_directory,
								(void*) ele->virtual_address, &pagetable);
			int j = 0;
			while (j != 1024) {
				if (pagetable[j] != 0) {
					break;
			...
			if (j == 1024) {
				free_frame(to_frame_info((uint32) pagetable & 0xffff000));
				pd_clear_page_dir_entry(e, ele->virtual_address);
			}
		}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	remove_from_LRU(e, virtual_address, size,1);
	remove_from_LRU(e, virtual_address, size,0);
