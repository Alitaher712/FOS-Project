...
//_______________________________________________________________
//_________________*___________*_________*_______________________
//________________*_*_________ *_________________________________
//_______________*__ *________ *_________*_______________________
//______________* * * *_______ *_________*_______________________
//_____________*_______*______ ****______*_______________________
//_______________________________________________________________
void push(struct Env * curenv, uint32 fault_va){
	struct WorkingSetElement* ali = LIST_FIRST(&(curenv->PageWorkingSetList));
		ali->virtual_address = fault_va;
		LIST_REMOVE(&curenv->PageWorkingSetList, ali);
		LIST_INSERT_HEAD(&curenv->ActiveList, ali);
}
void Placement(struct Env * curenv, uint32 fault_va ) {
	uint32 *ptr = NULL;
	struct Frame_Info*p = get_frame_info(curenv->env_page_directory,(void*) fault_va, &ptr);
	int ret = allocate_frame(&p);
		map_frame(curenv->env_page_directory, p, (void*) fault_va,PERM_WRITEABLE | PERM_PRESENT | PERM_USER);
	 push(curenv,fault_va);
	 ret = pf_read_env_page(curenv, (void*) fault_va);
	if (ret == E_PAGE_NOT_EXIST_IN_PF) {
		if (fault_va >= USTACKBOTTOM&&fault_va < USTACKTOP)
			 pf_add_empty_env_page(curenv, fault_va, 1);
		else
			panic("ERROR: Dose not Exist in the Stack or Page File");

	}

}
void remove_victim() {
	struct WorkingSetElement* vic = LIST_LAST(&curenv->SecondList);
	uint32 page_permissions = pt_get_page_permissions(curenv,vic->virtual_address);
	if ((page_permissions & PERM_MODIFIED)){
		uint32*p_t = NULL;
		struct Frame_Info*ptr = get_frame_info(curenv->env_page_directory,
				(void*) (vic->virtual_address), &p_t);
		pf_update_env_page(curenv, (void*) (vic->virtual_address),
				ptr);
	}
	unmap_frame(curenv->env_page_directory,(void*) vic->virtual_address);
	LIST_REMOVE(&curenv->SecondList, vic);
	LIST_INSERT_HEAD(&curenv->PageWorkingSetList, vic);
}
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	if (LIST_SIZE(&curenv->PageWorkingSetList) != 0) {
		 ...
		 else if (curenv->ActiveListSize == curenv->ActiveList.size&& curenv->SecondListSize != curenv->SecondList.size) {
			int flag = 0;
			LIST_REMOVE(&curenv->ActiveList, tail_active);
			pt_set_page_permissions(curenv,tail_active->virtual_address, 0,PERM_PRESENT);
			LIST_INSERT_HEAD(&curenv->SecondList, tail_active);
			if (LIST_SIZE(&(curenv->SecondList))!= 0) {
				struct WorkingSetElement *found_in_second;
				LIST_FOREACH(found_in_second, &(curenv->SecondList))
				{
					if (found_in_second->virtual_address == fault_va) {
						LIST_REMOVE(&curenv->SecondList, found_in_second);
						found_in_second->virtual_address=fault_va;
						pt_set_page_permissions(curenv,found_in_second->virtual_address, PERM_PRESENT,0);
						LIST_INSERT_HEAD(&curenv->ActiveList, found_in_second);
						return;
					}
				}
			}
	...
	else if (LIST_SIZE(&curenv->PageWorkingSetList)  == 0) {
		    LIST_REMOVE(&curenv->ActiveList, tail_active);
		    pt_set_page_permissions(curenv, tail_active->virtual_address, 0,PERM_PRESENT);
		    LIST_INSERT_HEAD(&curenv->SecondList, tail_active);
				struct WorkingSetElement *found_in_second=NULL;
				LIST_FOREACH(found_in_second, &(curenv->SecondList))
				{
					if (found_in_second->virtual_address == fault_va) {
						LIST_REMOVE(&curenv->SecondList, found_in_second);
